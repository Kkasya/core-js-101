...
function isInsideCircle(circle, point) {
  const c = Math.sqrt(Math.abs(circle.center.x - point.x) * Math.abs(circle.center.x - point.x)
    + Math.abs(circle.center.y - point.y) * Math.abs(circle.center.y - point.y));
...
function findFirstSingleChar(str) {
  ...
  str.split('').forEach((sym) => {
    if (str.indexOf(sym) === str.lastIndexOf(sym) && res === '') res = sym;
  });
  if (res === '') res = null;
  return res;
}
